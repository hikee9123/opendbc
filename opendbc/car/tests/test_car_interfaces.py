...
def get_fuzzy_params(car_name: str, example_idx: int) -> dict:
  rng = random.Random(f"{car_name}-{example_idx}")

  fingerprint = {}
  for addr in (0, 1, 0x7ff, 0x800)[:example_idx % 5]:
    fingerprint[addr] = DLC_TO_LEN[(example_idx + addr) % len(DLC_TO_LEN)]

  for _ in range((example_idx * 3) % 16):
    fingerprint[rng.randrange(0, 0x801)] = rng.choice(DLC_TO_LEN)

  car_fw = []
  fw_count = 0 if example_idx == 0 else 1 + ((example_idx * 5) % 12)
  for _ in range(fw_count):
    ecu, address, sub_addr = rng.choice(SORTED_ECUS)
    car_fw.append(structs.CarParams.CarFw(ecu=ecu, address=address, subAddress=sub_addr or 0,
                                          request=rng.choice(SORTED_REQUESTS)))

  return {
    'fingerprints': {0: fingerprint},
    'car_fw': car_fw,
    'alpha_long': bool(example_idx % 2),
  }


def get_fuzzy_car_interface(car_name: str, example_idx: int) -> CarInterfaceBase:
  params = get_fuzzy_params(car_name, example_idx)
...
def check_car_interface(car_name: str, car_interface: CarInterfaceBase):
...
def _make_car_test(car_name):
  def test(self):
    for example_idx in range(MAX_EXAMPLES):
      with self.subTest(example_idx=example_idx):
        car_interface = get_fuzzy_car_interface(car_name, example_idx)
        check_car_interface(car_name, car_interface)
