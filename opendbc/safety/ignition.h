...
static inline void ignition_can_hook(const CANPacket_t *msg, bool *ignition_can_out, uint32_t *ignition_can_cnt_out) {
  if (msg->bus == 0U) {
    int len = GET_LEN(msg);
    ...
    if ((msg->addr == 0x1F1U) && (len == 8)) {
      ...
      *ignition_can_out = (msg->data[0] & 0x2U) != 0U;
      *ignition_can_cnt_out = 0U;
    }
    ...
    if ((msg->addr == 0x152U) && (len == 8)) {
      ...
      if ((counter == ((prev_counter_rivian + 1) % 15)) && (prev_counter_rivian != -1)) {
        ...
        *ignition_can_out = ((msg->data[7] >> 4U) & 0x3U) == 1U;  // VDM_EpasPowerMode_Drive_On=1
        *ignition_can_cnt_out = 0U;
      }
    ...
    if ((msg->addr == 0x221U) && (len == 8)) {
      ...
      if ((counter == ((prev_counter_tesla + 1) % 16)) && (prev_counter_tesla != -1)) {
        ...
        *ignition_can_out = power_state == 0x3;  // VEHICLE_POWER_STATE_DRIVE=3
        *ignition_can_cnt_out = 0U;
      }
    ...
    if ((msg->addr == 0x9EU) && (len == 8)) {
      *ignition_can_out = (msg->data[0] >> 5) == 0x6U;
      *ignition_can_cnt_out = 0U;
    }
  }
